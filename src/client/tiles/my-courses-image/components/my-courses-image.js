...
function CourseBlock({course, fontSize, url, ...otherProps}) {
  ...
  return ( 
    <VBlock {...otherProps}>  
      ...
      <Box flex={3} key={course.id}>    
        {/* <SvgBox>
          <svg viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="45" fill="#9ec597" />
            <circle cx="90" cy="67" r="20" fill="#cf542a" />
            <text x="75" y="75" fill="white" fontSize="25" fontWeight="bold" verticalAlign="center">{notification}</text>
          </svg>
        </SvgBox> */}
        { course.announcementcount > 0 ? (
          <Badge number={course.announcementcount} color="#cf542a" position="top right" />
        ) : (
          null
        )
        }
        <ImageBox src={course.courseimageurl} onClick={course.courseimageurl} imageFit="stretch"></ImageBox>
...
export default function Payroll() {
  ...
  console.log("My Courses Image", responseBody);
